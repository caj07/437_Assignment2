...
std::pair<TAValue, TAValue>* TAValue::getPair() const {
	return getValue().getPair();
}

TAValue 
TAValue::getFirst() const {
	return getValue().getPair()->first;
}

TAValue
TAValue::getSecond() const {
	return getValue().getPair()->second;
}
void
TAValue::set(int v) {
	if (!theValue) {
...
void
TAValue::set(const TAPairType & t) {
	if (!theValue) {
		theValue = new TAPairValue(t);
		return;
	}
}
void
